...
public sealed class RunOrchestrator
{
	...
	public async Task ExecuteAsync(string runId, CancellationToken ct = default)
	{
		...
		var selectedSet = run.SelectedTests.Count > 0
			? new HashSet<PeripheralId>(run.SelectedTests)
			: null; // null = run all (backward compat)

		var orderedAdapters = _adapters
			.Where(a => selectedSet is null || selectedSet.Contains(a.PeripheralId))
		...
		IHardwareAdapter adapter, TestRun run, CancellationToken ct)
	{
		...
		Func<string, string, bool, Task> reportStep = async (cmd, output, isError) =>
		{
			...
			await _commandLog.SaveAsync(new CommandLogEntry
			{
